...
void usage() {
    std::printf(
        ...
        "  --gnss-baud N     UART rate for the GNSS receiver only\n"
        ...
        "  --gnss DEV        GNSS receiver UART (default /dev/serial0, NMEA 0183);\n"
        "                    opened this launch even if config says gnss.enabled = 0\n"
        "  --no-gnss         do not open the GNSS receiver at all\n"
        ...
        "the menu's Baud rates page is what keeps them. The GNSS receiver is the\n"
        "one peer whose port this app owns, so a wrong gnss.baud is probed across\n"
        "the table and the rate that answers is kept.\n"
        "\n"
